...
export function apiDataToFormData(
  ...
  // Extraire les URLs des documents
  const docUrls = data.documents
    ? getDocumentUrlsFromArray(data.documents)
    : { dpeImageUrl: null, gesImageUrl: null };
  ...
  return {
    ...
    transactionType:
      data.transactionType as PropertyFormData["transactionType"],
    ...
    showerRooms: data.characteristics?.showerRooms,
    toilets: data.characteristics?.toilets,
    floor: data.characteristics?.floor,
    floorIsRezDeChaussee: data.characteristics?.floorIsRezDeChaussee ?? false,
    ...
    dpeImageUrl: docUrls.dpeImageUrl,
    gesImageUrl: docUrls.gesImageUrl,
...
// Helper function to extract document URLs from array
function getDocumentUrlsFromArray(documents: { type: string; url: string }[]): {
...
export function formDataToApiData(data: PropertyFormData) {
  return {
    ...
    characteristics: {
      ...
      showerRooms: data.showerRooms || null,
      toilets: data.toilets || null,
      floor: data.floor || null,
      floorIsRezDeChaussee: data.floorIsRezDeChaussee ?? false,
